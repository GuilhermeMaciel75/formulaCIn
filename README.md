...
- **Trofeu()**:
> A classe troféu é uma subclasse da classe Item e ela contém o asset que representa o item troféu, a definição de escala e o ponto de referência do item. Além disso, a classe possui um método adicionar_trofeus() que adiciona no grupo de sprites para spawnar na tela.
- **Mapa()**:
...
> Em pontuacao.py. Imprime na parte direita da tela a pontuação de cada jogador de acordo com seus respectivos desempenhos no jogo durante a execução do jogo, além de imprimir imagens dos carros juntos das pontuações para melhor visualização.
- **def contar_tempo()**:
...
- **Pygame**:
> Esta biblioteca é um conjunto de módulos que foi desenvolvida para a escrita de jogos, ela permite que você crie jogos e programas multimídia a partir da linguagem python.
- **Random**:
...
> Uma estrutura condicional corresponde à uma função utilizada para determinar a veracidade de uma ou mais proposições, e a partir da resposta decidir qual bloco de código será executado à seguir. No projeto, as principais utilizações de estruturas condicionais são determinar qual ação foi tomada pelo jogador, a partir de um input do teclado, determinar se um player coletou algum powerup ou algum troféu e determinar se um player está colidindo com a parede do mapa.
- **Funções**:
